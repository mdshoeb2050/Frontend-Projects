    ...
    function renderTasks() {
        ...
        tasks.forEach((task, index) => {
            ...
            taskElement.innerHTML = `
                <div>${task.name}</div>
                <div class="progress-bar" data-progress="${task.progress}">
                    <svg width="60" height="60">
                        <circle cx="30" cy="30" r="25" fill="none" stroke="#ddd" stroke-width="5"></circle>
                        <circle cx="30" cy="30" r="25" fill="none" stroke="#4caf50" stroke-width="5" 
                                stroke-dasharray="${Math.PI * 25}" stroke-dashoffset="${Math.PI * 25 * (1 - task.progress / 100)}"></circle>
                    </svg>
                </div>
